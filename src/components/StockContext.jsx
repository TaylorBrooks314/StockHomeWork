...
export function StockContextProvider(props) {
  ...
  function addFollow(val) {
    ...
    let valid = followList.indexOf(val)


    if (valid === -1) {
      //list update
      setFollowList((prevList) => [...prevList, val])
      alert(`Following ${val}`)


    }
    else {
      setFollowList(followList.filter((Element) => { Element != val }))
      alert(`UnFollowing ${val}`)
  ...
  return (
    <StockContext.Provider value={{ stocks, currentStock, setCurrentStock, followList, addFollow }}>
      {props.children}
